...
function greet() {
  const name = getInput("Input your name");
  console.log(`\n Welcome ${name}!`);
  return name;
...
function guess(card1, card2) {
  console.log(`\n The current card is : ${card1.rank} of ${card1.suit}`)
  const input = getInput("Guess the next card! Will it be higher or lower than the current card? (Input h or l)").toLowerCase();
  ...
  if (input === "h" || input === "l") {
    ...
    console.log("No points earned for the round. Must choose between h or l!");
    return false;
  }
  }
...
function playGame() {
  ...
  while (score < 5 && score < deck.length) {
    ...
    if (guess(currentCard, nextCard)) {
      ...
      console.log("Incorrect! No points earned for the round.")
    }
    currentCard = nextCard;
  }
  deck.length != 0 ?  console.log(`\n Congratulations ${playerName}! you won!`) : console.log("Better luck next time! You ran out of cards!")
